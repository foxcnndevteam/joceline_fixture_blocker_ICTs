...
class LastLogsWindow(QtWidgets.QWidget):
    ...
    def getTests(self, qty: int):
        class Persona:
            def __init__(self, nombre, edad):
                self.nombre = nombre
                self.edad = edad
        dict = [Persona("Juan", 30), Persona("Pedro", 25)]
        
        return dict

    def updateTable(self, tests):
        self.table.clear()
        row_count = len(tests)
    ...
    def _extractKeys(self, tests) -> "list[str]":
        print("OLAAAAAAAAAA")
        print(tests)
